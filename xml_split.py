...
def next_file():
    ...
    if (not ending) and (cur_size > MAX_SIZE):
        ...
        # Open another file
        cur_idx += 1
        file_dir = os.path.join(out_dir, FMT % cur_idx)
        if not os.path.exists(file_dir):
            os.makedirs(file_dir)
        cur_file = open(os.path.join(out_dir, FMT % cur_idx + "/" + root + ext), 'wt', encoding='utf-8')
        shutil.copyfile("dblp.dtd", os.path.join(out_dir, FMT % cur_idx + "/" + root + ".dtd"))
        if xml_declaration is not None:
            cur_file.write('<?xml%s?>\n' % attrs_s(xml_declaration))
            cur_file.write('<!DOCTYPE %s SYSTEM "%s.dtd">\n' % (root, root))
